...
interface DynamicSlide8Props {
  data: {
    clientName: string;
    revenueGoal: number;
    pricePerClient: number;
    clientsPerThousand: number;
  };
}

const DynamicSlide8 = ({ data }: DynamicSlide8Props) => {
  const { clientName, revenueGoal, pricePerClient, clientsPerThousand } = data;

  const GOAL = typeof revenueGoal === "string" ? parseInt(String(revenueGoal).replace(/[$,]/g, ""), 10) : revenueGoal;
  const PRICE = typeof pricePerClient === "string" ? parseInt(String(pricePerClient).replace(/[$,]/g, ""), 10) : pricePerClient;
  const CLIENTS = typeof clientsPerThousand === "string" ? parseFloat(String(clientsPerThousand)) : clientsPerThousand;

  const revenuePerWeek = CLIENTS * PRICE;
  const weeksToGoal = Math.ceil(GOAL / revenuePerWeek);
  const totalPoints = Math.max(weeksToGoal + 2, 6);
  const useMonthlyLabels = totalPoints > 12;
  ...
  const weeksLabel = useMonthlyLabels
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div className="bg-black/80 border border-white/10 rounded-lg px-3 py-2 text-xs">
        ...
        <p className="text-white/40">{Math.min(d.week * CLIENTS, Math.ceil(GOAL / PRICE))} clients</p>
      </div>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="w-[72%] flex flex-col px-6 md:px-10 py-6 md:py-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mb-1">
          <span className="text-white/30 text-sm font-bold uppercase tracking-widest">
            {clientName}'s Revenue Roadmap
          </span>
        </motion.div>

        <motion.div initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="mb-4">
          ...
          <p className="text-white/50 text-xs md:text-sm mt-1 max-w-xl">
            ...
            you'd close at least {CLIENTS}. Here's what that compounds to.
          </p>
        </motion.div>

        {/* Chart */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7, duration: 0.5 }} className="flex-1 min-h-0 rounded-xl border border-white/10 bg-white/[0.03] p-3">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
              <defs>
                <linearGradient id="revenueGradDynamic" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="hsl(145,50%,45%)" stopOpacity={0.5} />
                  <stop offset="100%" stopColor="hsl(145,50%,45%)" stopOpacity={0.05} />
                </linearGradient>
              </defs>
              ...
              <Tooltip content={<CustomTooltip />} defaultIndex={goalWeekIndex} trigger="click" position={{ y: 80 }} />
        ...
        {/* Bottom callout */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5 }} className="mt-4 flex items-center gap-3 bg-[hsl(45,100%,55%)]/[0.08] border border-[hsl(45,100%,55%)]/20 rounded-xl px-4 py-2 relative z-10">
          <TrendingUp className="w-5 h-5 text-[hsl(45,100%,55%)] shrink-0" />
          <p className="text-white/70 text-xs md:text-sm leading-relaxed">
            At just{" "}
            <span className="text-[hsl(45,100%,55%)] font-bold">
              {CLIENTS} client{CLIENTS !== 1 ? "s" : ""} per 1,000 people
            </span>
            , you hit your
            <span className="text-[hsl(45,100%,55%)] font-bold">
            ...
            <span className="text-white font-bold">We already have an answer for you.</span>
          </p>
        </motion.div>
      </div>
    </motion.div>
